...
let getFilteredFacilities = (name) => {
    fetch(`https://localhost:8181/pas2/api/facilities/filter/${name}`)
        .then(response => {
            if (response.ok)
                return response;
        ...
        .then(data => {
            ...
            for (const facility of data) {
                str += "<tr>" +
                    ...
                    `<td><a href='updateFacility.html?id=${facility.id}'>Modify</a></td>` +
                    "</tr>";
            }
...
let getFacilities = () => {
    fetch("https://localhost:8181/pas2/api/facilities")
        .then(response => {
            if (response.ok)
                return response;
            throw new Error("error during get data");
...
let handleDeleteButtonClick = (id) => {
    ...
    if (confirm) {
        fetch(`https://localhost:8181/pas2/api/facilities/${id}`, {method: 'DELETE'})
            .then(response => {
                handleFilterChange();
            })
